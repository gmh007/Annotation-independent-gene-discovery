...
#####训练集预测混淆矩阵
svm_train_conf_matrix <- confusionMatrix(data = as.factor(svm_train_result), reference = as.factor(design_group1$Type), positive = "rod")
print(svm_train_conf_matrix)

#####测试集预测混淆矩阵
svm_test_conf_matrix <- confusionMatrix(data = as.factor(svm_test_result),  reference = as.factor(design_group2$Type), positive = "rod")
print(svm_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(svm_test_result, design_group2$Type ,positive = "rod" )
...
#####训练集预测混淆矩阵
nb_train_conf_matrix <- confusionMatrix(data = as.factor(nb_train_result), reference = as.factor(design_group1$Type), positive = "rod")
print(nb_train_conf_matrix)

#####测试集预测混淆矩阵
nb_test_conf_matrix <- confusionMatrix(data = as.factor(nb_test_result),  reference = as.factor(design_group2$Type), positive = "rod")
print(nb_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(nb_test_result, design_group2$Type ,positive = "rod" )
...
#####训练集预测混淆矩阵
ct_train_conf_matrix <- confusionMatrix(data = as.factor(ct_train_result), reference = as.factor(design_group1$Type), positive = "rod")
print(ct_train_conf_matrix)

#####测试集预测混淆矩阵
ct_test_conf_matrix <- confusionMatrix(data = as.factor(ct_test_result),  reference = as.factor(design_group2$Type), positive = "rod")
print(ct_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(ct_test_result, design_group2$Type ,positive = "rod" )
...
#####训练集预测混淆矩阵
dt_train_conf_matrix <- confusionMatrix(data = as.factor(dt_train_result), reference = as.factor(design_group1$Type), positive = "rod")
print(dt_train_conf_matrix)

#####测试集预测混淆矩阵
dt_test_conf_matrix <- confusionMatrix(data = as.factor(dt_test_result),  reference = as.factor(design_group2$Type), positive = "rod")
print(dt_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(dt_test_result, design_group2$Type ,positive = "rod" )
