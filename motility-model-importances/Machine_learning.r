...
# 训练模型
# 读取分组
design = read.table("flagellum_motility_group.txt", header = T, row.names = 1)
design$Type = as.factor(design$Type)
otu_table = read.table("Orthogroups.GeneCount_1000.tsv", header = T, row.names = 1)
...
library(pROC)
# 读取矩阵
raw_data <- read.csv('Orthogroups.GeneCount_1000.tsv', header = T, sep = '\t', row.names = 1, check.names = F)
raw_data <- t(raw_data)
dim(raw_data)
# 读取实验设计表
design <- read.csv('flagellum_motility_group.txt', header = T, sep = '\t', row.names = 1)
head(design)
...
#####训练集预测混淆矩阵
svm_train_conf_matrix <- confusionMatrix(data = as.factor(svm_train_result), reference = as.factor(design_group1$Type), positive = "Motility")
print(svm_train_conf_matrix)

#####测试集预测混淆矩阵
svm_test_conf_matrix <- confusionMatrix(data = as.factor(svm_test_result),  reference = as.factor(design_group2$Type), positive = "Motility")
print(svm_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(svm_test_result, design_group2$Type ,positive = "Motility" )
...
#####训练集预测混淆矩阵
nb_train_conf_matrix <- confusionMatrix(data = as.factor(nb_train_result), reference = as.factor(design_group1$Type), positive = "Motility")
print(nb_train_conf_matrix)

#####测试集预测混淆矩阵
nb_test_conf_matrix <- confusionMatrix(data = as.factor(nb_test_result),  reference = as.factor(design_group2$Type), positive = "Motility")
print(nb_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(nb_test_result, design_group2$Type ,positive = "Motility" )
...
#####训练集预测混淆矩阵
ct_train_conf_matrix <- confusionMatrix(data = as.factor(ct_train_result), reference = as.factor(design_group1$Type), positive = "Motility")
print(ct_train_conf_matrix)

#####测试集预测混淆矩阵
ct_test_conf_matrix <- confusionMatrix(data = as.factor(ct_test_result),  reference = as.factor(design_group2$Type), positive = "Motility")
print(ct_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(ct_test_result, design_group2$Type ,positive = "Motility" )
...
#####训练集预测混淆矩阵
dt_train_conf_matrix <- confusionMatrix(data = as.factor(dt_train_result), reference = as.factor(design_group1$Type), positive = "Motility")
print(dt_train_conf_matrix)

#####测试集预测混淆矩阵
dt_test_conf_matrix <- confusionMatrix(data = as.factor(dt_test_result),  reference = as.factor(design_group2$Type), positive = "Motility")
print(dt_test_conf_matrix)

########## 测试集混淆矩阵
# test_conf_matrix <- confusionMatrix(dt_test_result, design_group2$Type ,positive = "Motility" )
...
# 添加其他模型的ROC曲线
plot(svm_test_roc, add = TRUE, col = "royalblue", lwd = 2)
plot(nb_test_roc, add = TRUE, col = "forestgreen", lwd = 2)
plot(ct_test_roc, add = TRUE, col = "mediumorchid", lwd = 2)
plot(dt_test_roc, add = TRUE, col = "darkorange", lwd = 2)
