...
# 训练构建模型
set.seed(1001)
rf = randomForest(t(otu_sub), design_sub$Type, importance=TRUE, proximity=T, ntree = 2000, mtry=8)
print(rf)
# 保存模型
...
result = rfcv(t(otu_sub), design_sub$Type, cv.fold=10)
...
imp = head(imp, n=50)
# 反向排序X轴，让柱状图从上往下画
imp = imp[order(1:50,decreasing = T),]
#将imp的按第三列从小到大排序
#imp = imp[order(imp[,2]),]
# 取出列名
...
# 图1. feature重要性柱状图
library(ggplot2)
p=ggplot(data = imp, mapping = aes(x=Domain,y= vertebrate,fill=Domain)) + 
  geom_bar(stat="identity")+coord_flip()+theme_bw()+theme(legend.position = "none")
ggsave(p,filename = "imp_shape1.pdf",width = 16,height = 7)
...
#####训练集预测混淆矩阵
svm_train_conf_matrix <- confusionMatrix(data = as.factor(svm_train_result), reference = as.factor(design_group1$Type), positive = "vertebrate")
print(svm_train_conf_matrix)

#####测试集预测混淆矩阵
svm_test_conf_matrix <- confusionMatrix(data = as.factor(svm_test_result),  reference = as.factor(design_group2$Type), positive = "vertebrate")
...
#####训练集预测混淆矩阵
nb_train_conf_matrix <- confusionMatrix(data = as.factor(nb_train_result), reference = as.factor(design_group1$Type), positive = "vertebrate")
print(nb_train_conf_matrix)

#####测试集预测混淆矩阵
nb_test_conf_matrix <- confusionMatrix(data = as.factor(nb_test_result),  reference = as.factor(design_group2$Type), positive = "vertebrate")
...
#####训练集预测混淆矩阵
ct_train_conf_matrix <- confusionMatrix(data = as.factor(ct_train_result), reference = as.factor(design_group1$Type), positive = "vertebrate")
print(ct_train_conf_matrix)

#####测试集预测混淆矩阵
ct_test_conf_matrix <- confusionMatrix(data = as.factor(ct_test_result),  reference = as.factor(design_group2$Type), positive = "vertebrate")
...
#####训练集预测混淆矩阵
dt_train_conf_matrix <- confusionMatrix(data = as.factor(dt_train_result), reference = as.factor(design_group1$Type), positive = "vertebrate")
print(dt_train_conf_matrix)

#####测试集预测混淆矩阵
dt_test_conf_matrix <- confusionMatrix(data = as.factor(dt_test_result),  reference = as.factor(design_group2$Type), positive = "vertebrate")
